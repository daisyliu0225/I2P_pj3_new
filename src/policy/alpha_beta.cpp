...
Move Alpha_Beta::get_move(State *state, int depth){
  ...
  for(int i=0;i<sz;i++){
    int value = getalphabeta(state->next_state(state->legal_actions[i]), depth-1, mininf, maxinf, true);
    if(value > bestvalue){
      bestmove = i;
      bestvalue = value;
    }
...
int Alpha_Beta::getalphabeta(State *state, int depth, int alpha, int beta, bool maximizeplayer){
  ...
  if(maximizeplayer==true){
    ...
    for(int i=0;i<sz;i++){
      int value = getalphabeta(state->next_state(state->legal_actions[i]), depth-1, alpha, beta ,false);
    ...
    for(int i=0;i<sz;i++){
      int value = getalphabeta(state->next_state(state->legal_actions[i]), depth-1, alpha, beta, true);
